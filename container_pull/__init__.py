...
SAMPLE_REQUEST_BODY = {
    ...
    'delete': "(optional) delete the container if container state is succeeded"
}
...
def decipher_contgainer_logs(logs: str) -> tuple:
    if logs is not None and logs != "":
        with StringIO(logs) as log_stream:
            for line in log_stream:
                if any([code in line for code in ('[ERROR]', '[CRITICAL]')]):
                    return False, line
    return True, None
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try: 
        ...
        log_code, log_msg = decipher_contgainer_logs(logs)
        deleted = delete_container(client, rg, container_name) if (delete and state == 'succeeded') else False
        ...
        ret_code = 202
        if state == 'succeeded': ret_code = 200
        if state == 'failed': ret_code = 400
        if state == 'canceled': ret_code = 400
        if not log_code:
            ret_code = 404
            resp['status'] = 'error'
            resp['message'] = log_msg
        return func.HttpResponse(json.dumps(resp), status_code=ret_code, mimetype='application/json')
